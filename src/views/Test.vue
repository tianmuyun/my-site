  ...
  <div class="app">
    ...
    <div class="main">
      ...
      <div class="aside">
        ...
        <button v-if="webpUrl" @click="downloadWebP" class="download-btn">
          下载 WebP 图片
        </button>
      </div>
    </div>
...
export default {
  data() {
    return {
      webpUrl: null, // 存储转换后的 WebP 图片 URL
      imageInfo: null, // 存储图片尺寸信息
    };
  },
  methods: {
    // 处理图片上传
    ...
    convertToWebP(image) {
      ...
      this.webpUrl = webpUrl; // 设置转换后的 WebP 图片 URL
    },

    // 下载 WebP 图片
    downloadWebP() {
      const link = document.createElement("a");
      link.href = this.webpUrl;
...
.header {
  ...
  color: white; /* 文字颜色为白色 */
  padding: 20px 0; /* 上下内边距为30px，增加头部的高度 */
